...
sql <- glue('select * from arxiv_paper_repository.arxiv_paper_repository where date(submitted) between \'{floor_date(Sys.Date() - 90, \'month\')}\' AND \'{Sys.Date()}\' AND title <> \'\' order by date(submitted) desc')
